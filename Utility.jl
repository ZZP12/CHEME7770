...
function readExperimentData(file2data::String)
  aerobicX = readdlm(file2data)
  (datarow, dataxy) = size(aerobicX)
  AerobicXData = zeros(datarow, dataxy)
  for i = 1:datarow
    aerobicX[i,1] = replace(string(aerobicX[i,1]), ",", "")
    aerobicX[i,1] = parse(Float64, aerobicX[i,1])
  end
  return aerobicX
end
